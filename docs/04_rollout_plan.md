...
```mermaid
flowchart LR
    P1["Phase 1\nValidate\nWeek 1"] -->|VP sign-off| P2["Phase 2\nPilot\nWeeks 2–4"]
    P2 -->|60% unedited| P3["Phase 3\nFull Team\nWeeks 5–6"]
    P3 -->|70% unedited +\nclean deliverability| P4["Phase 4\nAuto-Send\nWeek 7+"]
```
...
**Go / no-go:** If a converted enterprise deal scores F, or if 20%+ of test emails contain incorrect facts, fix the scoring weights or LLM prompts before Week 2. If results roughly track with reality, proceed.
...
The tool generates the enrichment, score, pain points, and a draft email. An SDR reviews it, tweaks if needed, then manually clicks Send in their email client or CRM. The tool never sends anything autonomously during this phase.
...
|----------|---------------|
| How much do SDRs rewrite emails? | Edit distance — heavy rewrites signal something in the arc or data is systematically off |
...
| Which story arcs do SDRs trust? | Track which arcs they send as-is vs. always rewrite |
...
**Onboarding:**
- One 30-minute training session covering the SDR Guide
...
...wire up automated sending via the CRM's API (Outreach.io, Apollo, or HubSpot sequences). The tool pushes pre-scored emails into a send queue; the scheduler fires them at the right intervals. SDRs shift to reviewing replies and booking meetings rather than managing the queue.
